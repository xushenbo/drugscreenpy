...
def test_prompt_builders_match_original_workflow_wording():
    assert eds.tl.build_repurposing_indication_prompt("drug_a", "disease_x") == (
        "is drug_a used to treat disease_x? Just answer yes or no"
    )
    assert eds.tl.build_drug_indications_prompt("drug_a") == (
        "which diseases are drug_a used to treat? Limit answer within eight words"
    )
    assert eds.tl.build_repurposing_risk_factor_prompt("disease_a or disease_b", "disease_x") == (
        "is any disease in disease_a or disease_b a risk factor of disease_x? Just answer yes or no"
    )
    assert eds.tl.build_safety_symptom_prompt("drug_a", "disease_x") == (
        "is disease_x a symptom of any disorder treated by drug_a? Just answer yes or no"
    )
    assert eds.tl.build_safety_indication_prompt("drug_a", "disease_x") == (
        "is disease_x caused by any indication of drug_a? Just answer yes or no"
    )
    assert eds.tl.build_aging_prompt("disease_x") == "is disease_x more common as people age? Just answer yes or no"


def test_normalize_binary_answer_reduces_free_text_to_yes_no_unknown():
    assert eds.tl.normalize_binary_answer("Yes, definitely.") == "yes"
    assert eds.tl.normalize_binary_answer("NO.") == "no"
    assert eds.tl.normalize_binary_answer("unknown maybe") == "unknown"


def test_normalize_summary_answer_matches_original_cleanup():
    assert eds.tl.normalize_summary_answer("Asthma, and COPD.") == "asthma, or copd"
...
def test_prepare_unique_drug_disease_pairs_matches_original_unique_subset():
    ...
    pairs = eds.tl.prepare_unique_drug_disease_pairs(data)
...
def test_review_repurposing_indications_retries_error_like_responses():
    ...
    prompt = eds.tl.build_repurposing_indication_prompt("drug_a", "disease_x")
    llm = FakeLLM({prompt: ["Error: timeout", "Yes."]})

    reviewed = eds.tl.review_repurposing_indications(data, llm, max_attempts=3)
...
def test_summarize_drug_indications_normalizes_short_text_output():
    ...
    prompt = eds.tl.build_drug_indications_prompt("drug_a")
    llm = FakeLLM({prompt: "Asthma and COPD."})

    reviewed = eds.tl.summarize_drug_indications(data, llm)
...
def test_review_repurposing_risk_factors_uses_indication_summary_column():
    ...
    prompt = eds.tl.build_repurposing_risk_factor_prompt("asthma or copd", "pneumonia")
    llm = FakeLLM({prompt: "No"})

    reviewed = eds.tl.review_repurposing_risk_factors(data, llm)
...
def test_review_safety_workflows_match_original_columns():
    ...
    symptom_prompt = eds.tl.build_safety_symptom_prompt("drug_a", "disease_x")
    indication_prompt = eds.tl.build_safety_indication_prompt("drug_a", "disease_x")
    llm = FakeLLM({symptom_prompt: "yes", indication_prompt: "no"})

    symptoms = eds.tl.review_safety_symptoms(safety_pairs, llm)
    indications = eds.tl.review_safety_indications(safety_pairs, llm)
...
def test_review_safety_aging_short_circuits_short_exposures():
    ...
    prompt = eds.tl.build_aging_prompt("disease_long")
    llm = FakeLLM({prompt: "Yes"})

    reviewed = eds.tl.review_safety_aging(data, llm)
